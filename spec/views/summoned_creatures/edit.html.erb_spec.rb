...
RSpec.describe "summoned_creatures/edit", type: :view do
  before(:each) do
    @summoned_creature = assign(:summoned_creature, SummonedCreature.create!(
      :name => "MyString",
      :caster_id => 1,
      :type => "",
      :hp => 1,
  ...
  it "renders the edit summoned_creature form" do
    render

    assert_select "form[action=?][method=?]", summoned_creature_path(@summoned_creature), "post" do
